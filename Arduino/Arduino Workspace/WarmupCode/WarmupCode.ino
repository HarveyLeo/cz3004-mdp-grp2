    ...
    case 'W':
      {
        if (arg == 0){
          ...
          moveForward(1);
          delay(1000);
          MazeRunner_Flag = true;
          frontAlignment();
          MazeRunner_Flag = false;
          rotateRight(90);
          MazeRunner_Flag = true;
          frontAlignment();
          MazeRunner_Flag = false;
...
void moveForward(int distance) {
  ...
  switch (distance) {
    ...
    case 2: multiplier = 530; break;
    case 3: multiplier = 545; break;
    case 4: multiplier = 560; break;
    case 5: multiplier = 560; break;
    case 6: multiplier = 560; break;
    case 7: multiplier = 565; break;
    case 8: multiplier = 570; break;
    case 9: multiplier = 575; break;
    case 10: multiplier = 580; break;
    case 11: multiplier = 580; break;
    case 12: multiplier = 580; break;
    case 13: multiplier = 580; break;
    case 14: multiplier = 580; break;
    case 15: multiplier = 580; break;
    case 16: multiplier = 580; break;
    case 17: multiplier = 580; break;
    default: multiplier = 580; break;
...
int rotateLeft(int angle) {
  left_encoder_val = 0;
  right_encoder_val = 0;
  int pwm1 = 100, pwm2 = 100, output = 0;
  int angle_offset;
  ...
  else if ((angle > 45) && (angle <= 90)){   //8.92 When all fully charged (pid speed 300, no usb 5V)
    if(MazeRunner_Flag){
      pwm1 = 100;
      pwm2 = 100;
      angle_offset = angle * 9.25;
    }
    else angle_offset = angle * 9.23;            //9.33 When all fully charged, pid speed 200, no usb 5V
  }
...
int rotateRight(int angle) {
  ...
  if (angle <= 15) {
    ...
    angle_offset = angle * 8.9;
  else if ((angle > 45) && (angle <= 90)){ //8.95 When all fully charged (pid speed 300, no usb 5V)
     if(false){
      pwm1 = 200;
      pwm2 = 200;
      angle_offset = angle * 8.6;
    }    
    else angle_offset = angle * 9.23;          //9.33 When all fully charged, pid speed 200, no usb 5V
  }
  else if ((angle > 90) && (angle <= 360))
...
void distAlign() {
  while (1) {
    ...
    if (LFdistance > 19) md.setSpeeds(80, 80);
    else if (LFdistance > 15) md.setSpeeds(40, 40);
    else if (LFdistance < 15) md.setSpeeds(-40, -40);
    else break;
  }
  md.setBrakes(400, 400);
