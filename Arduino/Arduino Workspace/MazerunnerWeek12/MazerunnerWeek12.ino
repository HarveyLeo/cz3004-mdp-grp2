...
int rotateLeft(int angle) {
  ...
  if (angle <= 15) {
    ...
    if(MazeRunner_Flag){
      pwm1 = 300;
      pwm2 = 300;
      angle_offset = angle * 8.92;
    }
    else angle_offset = angle * 9.33;            //9.33 When all fully charged, pid speed 200, no usb 5V
  }
...
int rotateRight(int angle) {
  ...
  if (angle <= 15) {
    ...
    else angle_offset = angle * 9.34;          //9.33 When all fully charged, pid speed 200, no usb 5V
  }
  else if ((angle > 90) && (angle <= 360))
    angle_offset = angle * 8.9;
  else if ((angle > 360) && (angle <= 720))
...
void angleAlign() {
  while (1) {
    ...
    int error = LFreading - (RFreading-13);//Higher to the left, Lower to the right
